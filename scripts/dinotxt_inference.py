...
This script demonstrates how to use the DINOtxt model for zero-shot inference
...
# Add the parent directory to the path to import dinov3 modules
sys.path.append("../")

from dinov3.hub.dinotxt import dinov3_vitl16_dinotxt_tet1280d20h24l
from dinov3.data.transforms import make_classification_eval_transform

# Load the DINOtxt model and tokenizer
...
def get_zeroshot_classification_scores(image_pil, texts, class_names):
    """
    Get zero-shot classification scores on a sample image.
    ...
    Returns:
        similarity: Similarity scores between image and text prompts
    """
    image_preprocess = make_classification_eval_transform()
    image_tensor = torch.stack([image_preprocess(image_pil)], dim=0).cuda()
...
def get_patch_embeddings(image_tensor):
    """
    ...
    Returns:
        image_class_tokens: Class tokens from image
        image_patch_tokens: Patch tokens from image
        backbone_patch_tokens: Backbone patch tokens
        text_features_aligned_to_patch: Text features aligned to patch features
    """
    ...
    return image_class_tokens, image_patch_tokens, backbone_patch_tokens, text_features_aligned_to_patch

def compute_patch_similarity(image_patch_tokens, text_features_aligned_to_patch):
    """
    Compute similarity between image patches and text features.
...
imagenet_clip_class_names = [
    ...
    "flamingo", "little blue heron", "great egret", "bittern bird", "crane bird", 
    "limpkin", "common gallinule", "American coot", "bustard", "ruddy turnstone", 
    "dunlin", "common redshank", "dowitcher", "oystercatcher", "pelican", 
    "king penguin", "albatross", "grey whale", "killer whale", "dugong", "sea lion", 
    ...
    "brussels griffon", "Pembroke Welsh Corgi", "Cardigan Welsh Corgi", "Toy Poodle", 
    ...
    "common sorrel horse", "zebra", "pig", "wild boar", "warthog", "hippopotamus", 
    "ox", "water buffalo", "bison", "ram (adult male sheep)", "bighorn sheep", 
    "Alpine ibex", "hartebeest", "impala (antelope)", "gazelle", "arabian camel", 
    ...
    "snoek fish", "eel", "silver salmon", "rock beauty fish", "clownfish", "sturgeon", 
    "gar fish", "lionfish", "pufferfish", "abacus", "abaya", "academic gown", 
    ...
    "military hat (bearskin or shako)", "beer bottle", "beer glass", "bell tower", 
    "baby bib", "tandem bicycle", "bikini", "ring binder", "binoculars", "birdhouse", 
    "boathouse", "bobsleigh", "bolo tie", "poke bonnet", "bookcase", "bookstore", 
    "bottle cap", "hunting bow", "bow tie", "brass memorial plaque", "bra", 
    "breakwater", "breastplate", "broom", "bucket", "buckle", "bulletproof vest", 
    "high-speed train", "butcher shop", "taxicab", "cauldron", "candle", "cannon", 
    "canoe", "can opener", "cardigan", "car mirror", "carousel", "tool kit", 
    "cardboard box / carton", "car wheel", "automated teller machine", "cassette", 
    "cassette player", "castle", "catamaran", "CD player", "cello", "mobile phone", 
    "chain", "chain-link fence", "chain mail", "chainsaw", "storage chest", 
    "chiffonier", "bell or wind chime", "china cabinet", "Christmas stocking", 
    "church", "movie theater", "cleaver", "cliff dwelling", "cloak", "clogs", 
    "cocktail shaker", "coffee mug", "coffeemaker", "spiral or coil", 
    "combination lock", "computer keyboard", "candy store", "container ship", 
    ...
    "dining table", "dishcloth", "dishwasher", "disc brake", "dock", "dog sled", 
    "dome", "doormat", "drilling rig", "drum", "drumstick", "dumbbell", 
    "Dutch oven", "electric fan", "electric guitar", "electric locomotive", 
    "entertainment center", "envelope", "espresso machine", "face powder", 
    "feather boa", "filing cabinet", "fireboat", "fire truck", "fire screen", 
    "flagpole", "flute", "folding chair", "football helmet", "forklift", "fountain", 
    "fountain pen", "four-poster bed", "freight car", "French horn", "frying pan", 
    "fur coat", "garbage truck", "gas mask or respirator", "gas pump", "goblet", 
    ...
    "loupe magnifying glass", "sawmill", "magnetic compass", "messenger bag", 
    ...
    "mitten", "mixing bowl", "mobile home", "ford model t", "modem", "monastery", 
    "monitor", "moped", "mortar and pestle", "graduation cap", "mosque", 
    "mosquito net", "vespa", "mountain bike", "tent", "computer mouse", 
    "mousetrap", "moving van", "muzzle", "metal nail", "neck brace", "necklace", 
    "baby pacifier", "notebook computer", "obelisk", "oboe", "ocarina", "odometer", 
    ...
    "Polaroid camera", "pole", "police van", "poncho", "pool table", "soda bottle", 
    "plant pot", "potter's wheel", "power drill", "prayer rug", "printer", 
    "prison", "missile", "projector", "hockey puck", "punching bag", "purse", 
    "quill", "quilt", "race car", "racket", "radiator", "radio", "radio telescope", 
    ...
    "sewing machine", "shield", "shoe store", "shoji screen / room divider", 
    ...
    "military aircraft", "sink", "washing machine", "water bottle", "water jug", 
    "water tower", "whiskey jug", "whistle", "hair wig", "window screen", 
    "window shade", "Windsor tie", "wine bottle", "airplane wing", "wok", 
    "wooden spoon", "wool", "split-rail fence", "shipwreck", "sailboat", "yurt", 
    ...
    "ice cream", "popsicle", "baguette", "bagel", "pretzel", "cheeseburger", 
    "hot dog", "mashed potatoes", "cabbage", "broccoli", "cauliflower", "zucchini", 
...
# Zero-shot templates for ImageNet classification
...
def zeroshot_classifier(classnames, templates, tokenizer):
    ...
    Returns:
        zeroshot_weights: Classifier weights
    """
    with torch.no_grad():
        zeroshot_weights = []
...
def accuracy(output, target, topk=(1,)):
    ...
    Args:
        ...
        topk: Top-k accuracy to compute
...
def evaluate_imagenet_zeroshot(val_dataset, zeroshot_weights, batch_size=64, num_workers=8):
    """
    Evaluate zero-shot classification on ImageNet.
    
    Args:
        val_dataset: ImageNet validation dataset
        zeroshot_weights: Zero-shot classifier weights
...
if __name__ == "__main__":
    ...
    similarity = get_zeroshot_classification_scores(img_pil, texts, class_names)
    print("Similarity scores:", similarity)
    
    # Get patch embeddings
    image_preprocess = make_classification_eval_transform()
    image_tensor = torch.stack([image_preprocess(img_pil)], dim=0).cuda()
    image_class_tokens, image_patch_tokens, backbone_patch_tokens, text_features_aligned_to_patch = get_patch_embeddings(image_tensor)
    
    # Compute patch similarity
    pred_idx = compute_patch_similarity(image_patch_tokens, text_features_aligned_to_patch)
    print("Patch similarity computed")
    
    # Create zero-shot classifier for ImageNet
    ...
    print("Zero-shot classifier created")
